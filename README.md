...
4. Provide database, socials credentials and another staff in .env configuration file.
5. Enter `./vendor/bin/homestead make` in command line.
6. Let vagrant environment up in command line `vagrant up` and than `vagrant ssh`.
7. From project root (as usual you can get it by typing cd {project folder name}) under vagrant run `php artisan app:install` followed by `gulp --production` to setup the application. 
